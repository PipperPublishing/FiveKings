...
public class EvaluationComputerPlayer extends Player {
    ...
    protected PlayerHand findBestHand(final Hand.MeldMethod method, final boolean isFinalRound, final Card addedCard) {
        PlayerHand bestHand = this.hand;
        ...
        for (Card disCard : cardsWithAdded) {
            ...
            PlayerHand testHand = new PlayerHand(this.hand.roundOf, cards, disCard); //creates new hand with replaced cards
            testHand.meldAndEvaluate(method, isFinalRound);
            if (isFirstBetterThanSecond(testHand,bestHand,isFinalRound)) {
                bestHand = testHand;
                if (bestHand.getValueOrScore(isFinalRound) == 0) {
